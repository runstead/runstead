...
export interface PsqlPostgresControlPlaneClientOptions {
  command?: string;
}

export class PsqlPostgresControlPlaneClient implements PostgresControlPlaneClient {
  private pending:
    | {
        begin: string;
        end: string;
  ...
  static async connect(
    url: string,
    options: PsqlPostgresControlPlaneClientOptions = {}
  ): Promise<PsqlPostgresControlPlaneClient> {
    const child = spawn(
      options.command ?? "psql",
      [
        "--no-psqlrc",
        "--quiet",
        "--tuples-only",
    ...
    const client = new PsqlPostgresControlPlaneClient(child);
...
function sqlLiteral(value: unknown): string {
  ...
  if (json === undefined) {
    throw new Error("Unsupported Postgres query parameter");
  }
