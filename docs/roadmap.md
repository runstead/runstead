...
- `codex` command registration moved out of the CLI entrypoint into
  `commands/codex`.
- Codex auth login, status, logout, and model-list command handlers moved out
  of `commands/codex` into `commands/codex-actions`.
...
- Local agent text, Markdown, JSON, warnings, and audit-summary formatting
  moved out of `local-agent-report` into `local-agent-report-format`.
...
- Startup gate fixture schema loading and fixture-path discovery moved out of
  `startup-gate-test` into `startup-gate-test-fixtures`.
...
- Startup ops SOP generation moved out of the aggregate scale-ops module into
  `startup-scale-ops-sops`.
- Startup GTM artifact verification moved out of the aggregate scale-ops module
  into `startup-scale-ops-gtm`.
...
- Startup complete-check missing evidence, repo discovery, review-surface, and
  diagnostics signal helpers moved out of criteria construction into
  `startup-complete-check-missing`.
...
- Team control-plane migration SQL wrapper moved out of `team-control-plane`
...
- Domain validation command registration moved out of `commands/domain` into
...
- Startup metric snapshot write handler moved out of
  `commands/startup-measurement-snapshot` into
...
- Startup scale schedule-report, report, SOP generation, and GTM verification
  handlers moved out of `commands/startup-scale-ops` into
